    ...
    public class Raster<V, P> where V : struct, IVertex<V> where P : struct, IProjectedVertex<P>
    {
        ...
        public void DrawBezier(BezierCurve bezierCurve, bool drawExtras = false)
        {
            ...
            for (var i = 0; i < bezierCurve.CountOfSegments; i++)
            {
                ...
                DrawMesh(Manifold2<V>.Sphere(30, 30, points[0], .1f).ConvertTo(Topology.Lines));

                PixelShader = t => float4(.25f, .25f, .68f, 1);
                DrawMesh(Manifold2<V>.Sphere(30, 30, points[1], .1f).ConvertTo(Topology.Lines));
                DrawMesh(Manifold2<V>.Sphere(30, 30, points[2], .1f).ConvertTo(Topology.Lines));
                
                PixelShader = t => float4(.68f, .25f, .25f, 1);
                DrawMesh(Manifold2<V>.Sphere(30, 30, points[3], .1f).ConvertTo(Topology.Lines));
            }
        }
